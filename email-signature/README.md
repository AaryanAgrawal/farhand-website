...
Brand-consistent email signature for Farhand cofounders. Clone the template,
swap six fields, drop your headshot, paste into Gmail.

## What you're doing

1. Duplicate `public/email-signature/signature.html` → rename (e.g. `signature-amayr.html`).
2. Drop your square headshot into `public/email-signature/` (e.g. `amayr-headshot.jpg`).
3. Edit these six fields in your copy:
   ...
   - **LinkedIn** — `https://www.linkedin.com/in/aaaryan`
...
1. Load your deployed signature URL in Chrome.
...
5. **Uncheck** "Always match my default message font" (critical).
...
- `public/email-signature/signature.html` — Aaryan's signature (canonical template — clone this)
- `public/email-signature/preview.html` — rendered preview with mock email
- `public/email-signature/linkedin.svg`, `x.svg` — brand-color social icons (shared)
- `public/email-signature/aaryan-headshot.jpg` — Aaryan's headshot
- `email-signature/README.md` — this file (repo-only, not served on the web)
